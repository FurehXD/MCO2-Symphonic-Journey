...
void EndGame::updatePlayerDrawable(Player& player)
{
    this->playerShape = player.getShape();
    this->playerShape.setPosition(player.getPlayerPos());
}
std::vector<sf::Drawable*>& EndGame::getDrawables()
{
    return this->vecDrawables;
}

void EndGame::spawnNotes(int amount)
{
...
void EndGame::spawnBorders(int amount)
{
    //No borders for this level
}

std::vector<Collidable*>& EndGame::getCollidables()
{
